...
# Meetlocaties
psql -h $NDWDBHOST -U $NDWUSER -d ndw -f mst.sql > /dev/null
...
psql -h $NDWDBHOST -U $NDWUSER -d ndw -f tmp/wegvakken.sql > /dev/null

# Hectopunten
shp2pgsql -s 28992:4326  -I -c -W UTF-8 nwb/Hectopunten.shp hectopunten > tmp/hectopunten.sql
psql -h $NDWDBHOST -U $NDWUSER -d ndw -f tmp/hectopunten.sql > /dev/null

# VILD TMC-punten
shp2pgsql -s 28992:4326  -I -c -W UTF-8 ../data/vild/tmcpoint.shp tmcpoints > tmp/tmcpoint.sql
psql -h $NDWDBHOST -U $NDWUSER -d ndw -f tmp/tmcpoint.sql > /dev/null

# VILD table
psql -h $NDWDBHOST -U $NDWUSER -d ndw -f tmp/vild.sql > /dev/null

# Indexes
psql -h $NDWDBHOST -U $NDWUSER -d ndw -f indexes.sql > /dev/null
