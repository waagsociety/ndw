...
BEGIN
	...
	SELECT location, direction, distance AS offset FROM mst WHERE mst_id = _id  INTO meetpunt;
	RAISE NOTICE 'location: % direction: %, offset: %', meetpunt.location, meetpunt.direction, meetpunt.offset;
...
select wegvakken.wvk_id as wegvakid, wegnummer, hectomtrng, rijrichtng, afstand, beginkm from wegvakken JOIN hectopunten ON hectopunten.wvk_id = wegvakken.wvk_id where (wegnummer = wegId OR wegnummer = vildlocatie.roadnumber) AND hectomtrng = hLoc AND rijrichtng = wegDirection INTO paaltje;
